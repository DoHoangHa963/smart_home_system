...
public class MCUGatewayController {
    ...
    public ResponseEntity<ApiResponse<MCUPairingInitResponse>> initPairing(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success("MCU Gateway pairing initialized. Please select a home.", response));
    }
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.BAD_GATEWAY)
                    .body(ApiResponse.<String>error(
                            "Không thể kết nối với ESP32. Đảm bảo ESP32 đang online và trong cùng mạng WiFi.", null));
        }
    }
    ...
    public ResponseEntity<ApiResponse<DashboardDataResponse>> getDashboardData(
        ...
        try {
            // Trigger heartbeat trước để có data mới nhất
            try {
                mcuGatewayService.triggerHeartbeat(homeId);
                // Đợi một chút để ESP32 gửi heartbeat và backend xử lý
                Thread.sleep(500);
            } catch (Exception e) {
                log.debug("Failed to trigger heartbeat, continuing with existing data: {}", e.getMessage());
                // Tiếp tục lấy data dù trigger heartbeat thất bại
            }

            // Lấy full dashboard data
    ...
    /**
     * Proxy endpoint để gửi API Key đến ESP32
     * Tránh CORS issue khi frontend gọi trực tiếp ESP32
     */
    @Operation(summary = "Send API Key to ESP32 (Proxy)", description = "Proxy endpoint to send API Key to ESP32. Backend will forward the request to ESP32 to avoid CORS issues.")
    ...
    public ResponseEntity<ApiResponse<String>> sendApiKeyToESP32(
        ...
        log.info("Sending API Key to ESP32: ipAddress={}, mcuGatewayId={}",
                request.getEsp32IpAddress(), request.getMcuGatewayId());

        try {
            // Tạo URL ESP32 endpoint
            String esp32Url = "http://" + request.getEsp32IpAddress() + "/api/backend/pair";

            // Tạo request body
            String requestBody = String.format(
                    "{\"apiKey\":\"%s\",\"mcuGatewayId\":%d,\"homeId\":%d}",
                    request.getApiKey(),
            ...
            log.info("Successfully sent API Key to ESP32: {}", response);
            return ResponseEntity.ok(ApiResponse.success("API Key sent to ESP32 successfully", response));

        } catch (RestClientException e) {
            log.error("Failed to send API Key to ESP32: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_GATEWAY)
                    .body(ApiResponse.<String>error(
                            "Không thể kết nối với ESP32. Đảm bảo ESP32 đang online và trong cùng mạng WiFi.", null));
        } catch (Exception e) {
            log.error("Unexpected error sending API Key to ESP32: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.<String>error("Lỗi khi gửi API Key đến ESP32: " + e.getMessage(), null));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Object>> getAvailableGPIOPins(
        ...
        try {
            // Lấy MCU Gateway để có IP address
            MCUGatewayResponse mcuGateway = mcuGatewayService.getByHomeId(homeId);

            if (mcuGateway.getIpAddress() == null || mcuGateway.getIpAddress().trim().isEmpty()) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(ApiResponse.error(
                                "MCU Gateway IP address is not available. Please ensure ESP32 is online.", null));
            }

            // Gửi request đến ESP32 để lấy GPIO pins
            String esp32Url = "http://" + mcuGateway.getIpAddress() + "/api/gpio/available";
            ...
            // Parse JSON response từ ESP32
            com.fasterxml.jackson.databind.ObjectMapper mapper = new com.fasterxml.jackson.databind.ObjectMapper();
            Object gpioPins = mapper.readValue(response, Object.class);

            log.info("Successfully retrieved GPIO pins from ESP32 for homeId={}", homeId);
            return ResponseEntity.ok(ApiResponse.success("GPIO pins retrieved successfully", gpioPins));

        } catch (RestClientException e) {
            log.error("Failed to connect to ESP32 for homeId={}: {}", homeId, e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_GATEWAY)
                    .body(ApiResponse.error(
                            "Không thể kết nối với ESP32. Đảm bảo ESP32 đang online và trong cùng mạng WiFi.", null));
        } catch (Exception e) {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Lỗi khi lấy danh sách GPIO pins: " + e.getMessage(), null));
        }
    }
        ...
        try {
            ...
            if (automationType == null || enabled == null) {
                return ResponseEntity.badRequest()
                        .body(ApiResponse.<String>error("Missing automationType or enabled field", null));
            }
            ...
            if (!automationType.equals("AUTO_LIGHT") &&
                ...
                return ResponseEntity.badRequest()
                        .body(ApiResponse.<String>error(
                                "Invalid automationType. Must be AUTO_LIGHT, AUTO_FAN, or AUTO_CLOSE_DOOR", null));
            }
            ...
            return ResponseEntity.ok(ApiResponse.success(
                    String.format("Automation %s set to %s", automationType, enabled ? "ON" : "OFF"),
                    "success"));
        } catch (Exception e) {
            log.error("Failed to toggle automation for homeId={}: {}", homeId, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.<String>error("Failed to toggle automation: " + e.getMessage(), null));
        }
    }
    ...
    public ResponseEntity<ApiResponse<String>> setAutomationConfig(
        ...
        try {
            ...
            if (lightThreshold == null && tempThreshold == null && gasThreshold == null) {
                return ResponseEntity.badRequest()
                        .body(ApiResponse.<String>error("At least one threshold must be provided", null));
            }
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.<String>error("Failed to set automation config: " + e.getMessage(), null));
        }
    }
